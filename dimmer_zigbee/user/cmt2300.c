...
const unsigned char CMTBank[96] = { // // 软件自动配置生成的数据,用于初始化模块。模块上电进入睡眠模式。
...
void rf433_dev_tx(uint8_t datax, uint8_t datay)
{
    ...
    for(i=0; i<6; i++) {
        temp[i] = g_dev_mac.name[10+i];
    }
    for(i=0; i<6; i++) {
        temp[i+6] = g_dev_mac.controller_name[i];
    }
    ...
    vSetTxPayloadLength(31);
    vEnableWrFifo();
    bGoTx();

    for(i=0;i<31;i++) {
        FIFO_WriteByte(temp[i]);
    }
