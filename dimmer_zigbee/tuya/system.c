...
unsigned char zigbee_queue_buf[PROTOCOL_HEAD + ZIGBEE_UART_QUEUE_LMT];        //串口队列缓存
unsigned char zigbee_uart_rx_buf[PROTOCOL_HEAD + ZIGBEE_UART_RECV_BUF_LMT];   //串口接收缓存
unsigned char zigbee_uart_tx_buf[PROTOCOL_HEAD + ZIGBEE_UART_SEND_BUF_LMT];   //串口发送缓存

unsigned char *queue_in;
unsigned char *queue_out;
volatile uint8_t queue_total_data = 0;
...
//    return dest;
    ...
    zigbee_uart_tx_buf[DATA_START + dest] = byte;
...
void zigbee_uart_write_frame(unsigned char fr_type, unsigned short len)
{
    ...
    zigbee_uart_tx_buf[SEQ_HIGH] = (uint8_t)(seq_num >> 8) ;
    zigbee_uart_tx_buf[SEQ_LOW]  = (uint8_t)(seq_num);
    zigbee_uart_tx_buf[FRAME_TYPE] = fr_type;
    zigbee_uart_tx_buf[LENGTH_HIGH] = (uint8_t)(len >> 8);
    zigbee_uart_tx_buf[LENGTH_LOW]  = (uint8_t)(len);

    seq_num++;
    seq_num_set(seq_num);
    len += PROTOCOL_HEAD ;
    check_sum = get_check_sum(zigbee_uart_tx_buf, len - 1);
    zigbee_uart_tx_buf[len - 1] = check_sum;

//    zigbee_uart_write_data((unsigned char *)zigbee_uart_tx_buf, len);

    uart_send_buf(zigbee_uart_tx_buf, len);
}
...
unsigned char Queue_Read_Byte(void)
{
    ...
    if(queue_total_data > 0) {
        value = zigbee_queue_buf[--queue_total_data ];
//        queue_total_data--;
    }
                              ...
                              unsigned char req_data_len)
{
    ...
    length = set_zigbee_uart_byte(length, (uint8_t)(image_offset >> 24));
    length = set_zigbee_uart_byte(length, (uint8_t)(image_offset >> 16));
    length = set_zigbee_uart_byte(length, (uint8_t)(image_offset >> 8));
    length = set_zigbee_uart_byte(length, (uint8_t)(image_offset));
...
void data_handle(unsigned short offset)
{
    ...
    total_len  = zigbee_uart_rx_buf[offset + LENGTH_HIGH] << 8;
    ...
    case ZIGBEE_STATE_CMD: {
        uint8_t current_state = zigbee_uart_rx_buf[offset + DATA_START];
        zigbee_state_cmd_handle(current_state);
        if(current_state == 1) {
            g_zig_net_sta = ZIG_NET_STA_OK;
        }
    }
    break;
