...
unsigned char mcu_dp_raw_update(unsigned char dpid, const unsigned char value[], unsigned short len)
{
    ...
    //
    length = set_zigbee_uart_byte(length, (uint8_t)(len >> 8));
    length = set_zigbee_uart_byte(length, (uint8_t)(len));
    //
...
unsigned char mcu_dp_value_update(unsigned char dpid, unsigned long value)
{
    ...
    //
    length = set_zigbee_uart_byte(length, (uint8_t)(value >> 24));
    length = set_zigbee_uart_byte(length, (uint8_t)(value >> 16));
    length = set_zigbee_uart_byte(length, (uint8_t)(value >> 8));
    length = set_zigbee_uart_byte(length, (uint8_t)(value));
...
//  #error " 请在main函数中添加zigbee_protocol_init()完成zigbee协议初始化,并删除该行"
    queue_in = &zigbee_queue_buf[0];
    queue_out = &zigbee_queue_buf[0];
    queue_total_data = 0;
}
...
//  #error "请在串口接收中断中调用uart_receive_input(value),串口数据由MCU_SDK处理,用户请勿再另行处理,完成后删除该行"
    ...
    if(queue_total_data >= (PROTOCOL_HEAD + ZIGBEE_UART_QUEUE_LMT)) {
        queue_total_data = 0;
    }

    zigbee_queue_buf[queue_total_data] = value;
    queue_total_data++;
}
...
//    #error "请直接在main函数的while(1){}zigbee_uart_service(),调用该函数不要加任何条件判断,完成后删除该行"
    ...
    while((rx_in < sizeof(zigbee_uart_rx_buf)) && queue_total_data) {
    ...
    if(rx_in > 0) {
        if((offset == 0)  && (rx_in >= sizeof(zigbee_uart_rx_buf))) {
            ...
            offset++;
            rx_in -= offset;
//            my_memcpy(zigbee_uart_rx_buf, zigbee_uart_rx_buf + offset, rx_in);
        } else {
            if(offset) {
                my_memcpy(zigbee_uart_rx_buf, zigbee_uart_rx_buf + offset, rx_in);
            }
