...
void Linkage_open_close(uint8_t key,uint8_t state) 
{
  ...
  case 1:
    ...
    Relay_open_Close_Config(CH1_PORT,CH1_PIN,state);  //继电器通道1设置低电平。
    break;
  case 2:
    LED_open_Close_Config(1,state);  //LED2输出0
    Relay_open_Close_Config(CH2_PORT,CH2_PIN,state);  //继电器通道2设置低电平。
    break;
  case 3:
    LED_open_Close_Config(2,state);  //LED3输出0
    Relay_open_Close_Config(CH3_PORT,CH3_PIN,state);  //继电器通道3设置低电平。
    break;
  case 4:
    LED_open_Close_Config(3,state);  //LED4输出0
    Relay_open_Close_Config(CH4_PORT,CH4_PIN,state);  //继电器通道4设置低电平。
    break;
  default:
    break;
  }
