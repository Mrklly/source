...
void PWM_timer1(void)
{
  ...
  Gpio_Init(GpioPortA, GpioPin8, &stcTIM1Port);
...
{  
  ...
  case 1:
    ...
    break;
  case 2:
    Tim3_M23_CCR_Set(Tim3CCR0B, TIM1_Pulse);                //2键
    break;
  case 3:
    Tim3_M23_CCR_Set(Tim3CCR1B, TIM1_Pulse);                //3键
    break;
  case 4:
    Tim3_M23_CCR_Set(Tim3CCR2B, TIM1_Pulse);                //4键
    break;
  default:
    break;
  }   
...
void PWM_Contorl_LED(uint8_t ch, uint8_t ch_state)
{
  if(ch_state==1)
  {
    ...
    if(ch == 1) {
        back_LED_open_Close_Config(5, 0);
    } else if(ch == 4) {
        back_LED_open_Close_Config(6, 0);
    }
    if(count[ch-1]==0)
    {
      pwm_flag[ch-1]=0x02;
