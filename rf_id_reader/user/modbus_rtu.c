...
uint8_t uart_read_packet(uint8_t *buff)
{
    ...
    if((g_idx < 2 + 4 + register_number + 2) || (g_idx == 0)) {   // waiting for a whole frame to be received
        return 0;
    }

    memcpy(buff, g_rx_buff, (2 + 4 + register_number + 2));// backup the whole frame

    if((g_rx_buff[0] != 0x77) || (g_rx_buff[1] != 0x99)) {
        g_idx = 0;
        return 0;
    }
