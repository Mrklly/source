...
INTERRUPT_HANDLER(UART1_RX_IRQHandler, 18)
{
    if(UART1_GetITStatus(UART1_IT_RXNE) != RESET) {
        ...
        g_uart_buf[g_uart_idx] = (uint8_t)(UART1->DR);
        g_uart_idx++;
...
/**
 ...
 {
        static volatile uint16_t timer_cont = 0;

        if(timer_cont == 30) {   // 20ms irq
