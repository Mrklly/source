 ...
 INTERRUPT_HANDLER(TIM2_UPD_OVF_BRK_IRQHandler, 13)
{
    ...
    if(pwm_flag[0]==0x01) {       //第一路调光。按键有变化。
        PWM_Contorl_LED(1,key_led_state[0]);
    }
    if(pwm_flag[1]==0x01) {       //第二路调光。
        PWM_Contorl_LED(2,key_led_state[1]);
    }
    if(pwm_flag[2]==0x01) {       //第三路调光。
        PWM_Contorl_LED(3,key_led_state[2]); 
    }
    if(pwm_flag[3]==0x01) {       //第四路调光。
        PWM_Contorl_LED(4,key_led_state[3]);  
    }
